export default class Page {
    ...
    */
    ...
    async compareUrl(newUrl: string) {
        const currentUrl = await browser.getUrl();
        await console.info('Waiting for current URL ' + currentUrl);

        if (newUrl === currentUrl) {
            console.info(`${currentUrl} is equal to ${newUrl}`);
            return true;
        } else {
            console.info(`${currentUrl} is not equal to ${newUrl}`);
            return false;
        }
    }
